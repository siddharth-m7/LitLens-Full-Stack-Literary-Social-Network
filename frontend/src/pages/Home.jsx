      ...
      <div className="flex-1 flex items-center max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <div className="w-full flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          <div className="flex-1 max-w-2xl">
            ...
            {/* Pill badge */}
            <span className="inline-flex items-center gap-2 bg-[#F0EAD6] border border-[#DDD3B8] rounded-full px-4 py-1.5 text-xs font-semibold text-[#8B7355] uppercase tracking-widest mb-7">
              <span className="w-1.5 h-1.5 rounded-full bg-[#8B7355] shrink-0"></span>
          ...
          <div className="hidden lg:block flex-shrink-0 w-[26rem] h-[22rem] relative">
            ...
            {/* Main card */}
            <div className="relative w-full h-full bg-gradient-to-br from-[#F5EFE3] to-[#EDE5D0] border border-[#DDD3B8] rounded-3xl overflow-hidden flex flex-col items-center justify-center gap-6 shadow-lg">

              {/* Decorative top-right circle */}
              <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full bg-[#DDD3B8]/40"></div>
              <div className="absolute -bottom-6 -left-6 w-24 h-24 rounded-full bg-[#C8B89A]/30"></div>
              ...
              <div className="flex items-end gap-3 relative z-10">
                {BOOKS.map((book, i) => (
                  <div
                    key={i}
                    className={`${book.h} ${book.w} ${book.bg} rounded flex items-end justify-center pb-3 shadow-md`}
                  >
                    <div className="w-1 h-8 bg-white/25 rounded-full"></div>
                  </div>
                ))}
              </div>
              ...
              <div className="relative z-10 flex flex-col items-center gap-1">
                <div className="flex items-center gap-1.5">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className="w-3.5 h-3.5 text-[#8B7355] fill-current" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                    </svg>
                  ))}
                </div>
                <p className="text-xs text-[#8B7355] font-semibold tracking-widest uppercase">Your Next Read Awaits</p>
              </div>
